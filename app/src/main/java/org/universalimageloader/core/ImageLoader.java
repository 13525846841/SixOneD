...
public class ImageLoader {
    ...
    private static final String ERROR_NOT_INIT = "ImageLoader must be initialize with configuration before using";
    ...
    public static ImageLoader getInstance() {
        if (instance == null) {
            synchronized (ImageLoader.class) {
                if (instance == null) {
                    instance = new ImageLoader(AppContext.getApplication());
                }
            }
        }
        return instance;
    ...
    /**
     * 初始化下载单例模式
     * Initializes ImageLoader's singleton getInstance with configuration. Method should be called <b>once</b> (each
...
//		if(!HStringUtil.isEmpty(uri) && uri.startsWith("assets/customerIcons"))
//			uri=null;
        String uri = AppContext.getApiRepository().LOADCUSHIDPICSERVLET42 + "?customerid=" + id + "&isgroup=0";
        ;
        if ("1".equals(sex) || "M".equalsIgnoreCase(sex)) {//男
            displayImage(uri, imageView, maleImageOptions);
        } else if ("2".equals(sex) || "W".equalsIgnoreCase(sex)) {//女
            ...
            uri = AppContext.getApiRepository().LOADCUSHIDPICSERVLET42 + "?customerid=" + id + "&isgroup=1";
            displayImage(uri, imageView, groupImageOptions);
        } else if ("5".equals(sex)) {//没有默认图片
            displayImage(uri, imageView, notImageOptions);
        } else {//未知 || "X".equalsIgnoreCase(type)
    ...
    public void displayImageFromId(String id, ImageView imageView) {
        String uri = AppContext.getApiRepository().URL_QUERYHEADIMAGE_FROM_ID + "?customerid=" + id + "&isgroup=0";
        ;
        displayImage(uri, imageView, unkounwImageOptions);
    }
...
//	}
     ...
     *
     * @param context      Application context (used for creation of fake {@link ImageView})
     * @param uri          fileassets:///image.png
