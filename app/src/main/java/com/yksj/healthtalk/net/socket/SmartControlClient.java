...
public class SmartControlClient {
    ...
    private static int PORT = Configs.SOCKET_PORT;//socket端口
    public static String IP = Configs.SOCKET_IP;//socket地址
    ...
    private LoginStatus loginState = LoginStatus.NONE;//登录状态-1未登录,0登录中,1登录成功,2资料加载完成
    private long lastConnectTime;
    private String userName;
    private String password;
    private String userMd5Id;
    private final String osVersion = AppUtils.getAppVersionName();
    private final String osType = "1";
    private XsocketHanlder mListener;
    private final SocketManager mSocketManager;
    //	private String CONSULTATION_CENTER_ID=AppContext.APP_CONSULTATION_CENTERID;
...
//        Resources resources = AppContext.getAppResources();
...
//        mSocketManager = SocketManager.getSocketManager(mListener, resources.getString(R.string.smart_ip), resources.getString((R.string.smart_port)));
...
//        mSmartFox.addEventListener(SFSEvent.HANDSHAKE, mListener);
    ...
    public synchronized void doSendExtentionReq(SocketParams sfsObject) {
        ...
        sfsObject.putUtfString("CONSULTATION_CENTER_ID", AppContext.APP_CONSULTATION_CENTERID);
...
//                if (AppContext.getApplication().isNetWork()) connect();
//            }
//        }
//    }
...
//        }
    ...
    public synchronized CustomerInfoEntity getInfoEntity() {
        return LoginBusiness.getInstance().getLoginEntity();
//        try {
//            if (mLoginEntity == null) {
//                String parame = SharePreHelper.getLoginUserInfo();
//                String[] str = SharePreHelper.fatchUserLoginCache();
...
//                if (getLoginState() != LoginStatus.LOGIN_OK) {
//                    setLoginState(LoginStatus.LOGINING);
//                }
//                if (isConnected()) {
//                    onLoginClick();
//                } else {
...
//        }
    ...
    public synchronized void setCustomerInfoEntity(CustomerInfoEntity mEntity) {
        LoginBusiness.getInstance().setLoginInfo(mEntity);
//        this.mLoginEntity = mEntity;
//        if (mLoginEntity != null) {
//            setLoginState(LoginStatus.LOGIN_OK);
...
//            sendLoadUserInfo();
    ...
    public synchronized boolean isLogined() {
        return loginState == LoginStatus.LOGIN_OK;
    }
...
/*	public synchronized void senPingPongRequest(){
    ...
    public synchronized void sendKeepConnect() {
        HeartServiceManager.getInstance().sendHeartMsg();
    }
    ...
    public synchronized void setUserPassword(String userName, String password) {
        ...
        ApiService.addHttpHeader("username", this.userName);
    ...
    public synchronized void updateUserPassword(String password) {
        ...
        ApiService.addHttpHeader("password", password);
    }

    public LoginStatus getLoginState() {
        return LoginBusiness.getInstance().getLoginState();
    }
    ...
    public static final int LOGIN_CODE = 1001;//登录
    public static final int REPLY_LOGIN_CODE = 1003;//登录
    ...
    public static final int CHATTING_GROUP_MESSAGE_RECEIVE = 9045;// 接受群聊消息 系统通知
    public static final int CHATTING_SINGLE_MESSAGE_RECEIVE = 9019;// 接受单聊消息 系统通知
    public static final int CHATTING_MESSAGE_SENDSTATE = 9018;// 消息发送状态返回
    ...
    public static final int SERVICE_SINGLE_RECIEVE_MSG = 7056;// 单聊接收
