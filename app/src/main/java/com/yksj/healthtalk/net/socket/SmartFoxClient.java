...
//		this.ip = ip;
    ...
    public static String getLoginUserId() {
        if (LoginBusiness.getInstance().getLoginEntity() != null) {
            return LoginBusiness.getInstance().getLoginEntity().getId();
        } else {
            return "";
        }
     ...
     * @param type
     */
    public static void sendChatMessage(MessageEntity entity, int type, int duration) {
...
//        params.putUtfString("customerId", smartFoxClient.getUserId());
        params.putUtfString("customerId", DoctorHelper.getId());
        ...
        params.putUtfString("Object_Type", "10");//会诊标记
        if (consulId != null)
...
//		if(fileName != null)SocketParams.putUtfString("fileName", fileName);
        ...
        if (entity.getGroupType() == 1) {
            SocketManager.sendSocketParams(params, SmartControlClient.SIX_ONE_SEND_MSG);
        } else if (entity.getGroupType() == 2) {
            SocketManager.sendSocketParams(params, SmartControlClient.SOCKET_GROUP_CHAT);
        } else if (entity.getGroupType() == 3) {
            SocketManager.sendSocketParams(params, SmartControlClient.SERVICE_SINGLE_SEND_MSG);
        } else {
            SocketManager.sendSocketParams(params, SmartControlClient.SOCKET_SINGLE_CHAT);
        }
//		smartFoxClient.doSendExtentionReq(params);
    }
