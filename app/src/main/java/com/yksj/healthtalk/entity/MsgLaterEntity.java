...
public class MsgLaterEntity implements Serializable {
    ...
    public static List<MsgLaterEntity> parseToList(JSONArray array) {
        ...
        for (int i = 0; i < array.length(); i++) {
            ...
            if (SmartFoxClient.getLoginUserId().equals(item.optString("TARGET_ID")))
                entity.setSendId(item.optString("SEND_ID"));
            else
                entity.setSendId(item.optString("TARGET_ID"));

            if (SmartFoxClient.helperId.equals(item.optString("SEND_ID"))
                    || SmartFoxClient.helperId.equals(item.optString("TARGET_ID"))) {
...
//			entity.setHeaderPath(item.optString("NUMSsssss"));
