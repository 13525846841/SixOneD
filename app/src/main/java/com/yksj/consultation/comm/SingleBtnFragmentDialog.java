...
public class SingleBtnFragmentDialog extends DialogFragment {
    ...
    //显示病历流程单按钮对话框
    public static Dialog showSinglebtnFloe(Context context,String textName,String textContent,String btn1,final OnClickSureBtnListener singlelistener,View.OnClickListener textClickListener){
        final Dialog dialog = new Dialog(context,R.style.translucent_dialog);
        dialog.setCancelable(false);//使双按钮对话框点击屏幕不可被取消
        dialog.setCanceledOnTouchOutside(false);
        dialog.setOnKeyListener(new DialogInterface.OnKeyListener() {
        ...
        dialog.setContentView(R.layout.flow_single_dialog);
        TextView textNameTag = (TextView)dialog.findViewById(R.id.flow_single_title);
        TextView tvContent= (TextView) dialog.findViewById(R.id.flow_single_content);
        tvContent.setText(textContent);
        Button tvClick= (Button) dialog.findViewById(R.id.flow_single_inner_click);
        tvClick.setOnClickListener(textClickListener);
        Button close = (Button)dialog.findViewById(R.id.close_way);
        Button button1 = (Button)dialog.findViewById(R.id.flow_single_click);
        close.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                dialog.dismiss();
