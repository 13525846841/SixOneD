...
public class MessageHistoryListAdapter extends BaseSwipeAdapter {
    ...
    public void fillValues(final int position, View convertView) {
        ...
        if (SmartFoxClient.helperId.equals(item.getSendId())) {//多美助理
            mHead.setImageResource(R.drawable.ic_launcher);
...
//			mInstance.displayImageFromId(item.getSex(),  item.getSendId(), mHead);
//			String picUrl = AppContext.getHttpUrls().URL_QUERYHEADIMAGE + item.getHeaderPath();
//			Picasso.with(context).load(picUrl).connectError(R.drawable.default_head_mankind).placeholder(R.drawable.default_head_mankind).into(mHead);
            viewFinder.setText(R.id.name, item.getNickName());
        }
