...
public class SettingPassWordUI extends BaseActivity implements OnClickListener {
    ...
    private LinearLayout codell;//title_below1
...
//			}
    ...
    private void comfirm() {
        ...
        if ("".equals(pasnew)|| "".equals(pasagain)) {
            ...
            DialogUtils.getErrorDialog(SettingPassWordUI.this,getString(R.string.app_name),
            ...
            if(getIntent().hasExtra("CODE")){
                ApiService.OKHttpUpdateFindPassword(intentPhone, password, intentCode, new MyApiCallback<org.json.JSONObject>(this) {
                    @Override
                    ...
                    public void onResponse(org.json.JSONObject response) {
                        ...
                        }else if ("0".equals(response.optString("code"))){
                            ToastUtil.showShort(response.optString("message"));
                        }
                    }
                },this);
            }else {
                ApiService.OKHttpUpdatePassword(LoginBusiness.getInstance().getLoginEntity().getId(), password, codeEdit.getText().toString().trim(), new MyApiCallback<org.json.JSONObject>() {
                    @Override
                    public void onError(Request request, Exception e) {
...
//			ApiService.doHttpChangePassword(SmartFoxClient.getSmartFoxClient().getUserId(), password, new AsyncHttpResponseHandler(this){
//				@Override
...
//			});
    ...
    private void getAuthCode() {
        ...
        if ( ValidatorUtil.checkMobile(phone)) {
            ApiService.OKHttpSendUpdatePasswordCode(LoginBusiness.getInstance().getLoginEntity().getId(), new MyApiCallback<JSONObject>(this) {
                @Override
                public void onError(Request request, Exception e) {
