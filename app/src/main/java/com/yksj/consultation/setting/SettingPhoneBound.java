...
public class SettingPhoneBound extends BaseActivity implements
    ...
    private void initView() {
        ...
        if (getIntent().hasExtra("ALTERBAND")) {
            isAlter = false;
            titleTextV.setText("修改绑定手机");
            findViewById(R.id.title_below).setVisibility(View.VISIBLE);
        }
        ...
        if (LoginBusiness.getInstance().getLoginEntity() != null) {
            phoneNum = LoginBusiness.getInstance().getLoginEntity().getPoneNumber();
        }
        phone1.setText(phoneNum);//code_btn
...
//			break;
    ...
    private void UnBund() {
        ...
        ApiService.doHttpUnPhoneBind(customerid, new AsyncHttpResponseHandler(this) {
            @Override
            ...
            if (!TextUtils.isEmpty(phone)) {
                if (!TextUtils.isEmpty(code)) {
                    if (phone != null && phone.length() == 11 && ValidatorUtil.checkMobile(phone)) {
                        ApiService.doHttpConsultationSetPhoneBound(phone, code, SmartFoxClient.getSmartFoxClient().getUserPas(), customerid,
...
//		String phone = mInputNumber.getText().toString();
        ...
        if (isAlter) {
            if (ValidatorUtil.checkMobile(phoneStr)) {
                ...
                ApiService.OKHttpSendBindPhoneNumCode(phoneStr, new MyApiCallback<JSONObject>(this) {
                    @Override
            ...
            if (ValidatorUtil.checkMobile(phoneStr)) {
                ApiService.OKHttpSendUpdatePhoneNumCode(phoneStr, new MyApiCallback<JSONObject>(this) {
                    @Override
                    public void onError(Request request, Exception e) {
    ...
    private void upDataPhone() {
        ...
        if (isAlter) {
            ApiService.OKHttpBindPhoneNum(LoginBusiness.getInstance().getLoginEntity().getId(), phoneStr, codeStr, new MyApiCallback<JSONObject>(this) {
                @Override
                public void onError(Request request, Exception e) {
                ...
                public void onResponse(JSONObject response) {
                    ...
                    if ("1".equals(response.optString("code"))) {
                        ...
                        String message = response.optString("result");
                        ToastUtil.showToastPanl(message);
                        LoginBusiness.getInstance().getLoginEntity().setPoneNumber(phoneStr);
                        finish();
                    } else {
            ...
            String lastPhone = phone1.getText().toString().trim();
            ApiService.OKHttppdatePhoneNum(lastPhone, LoginBusiness.getInstance().getLoginEntity().getId(), phoneStr, codeStr, new MyApiCallback<JSONObject>(this) {
                @Override
                public void onError(Request request, Exception e) {
