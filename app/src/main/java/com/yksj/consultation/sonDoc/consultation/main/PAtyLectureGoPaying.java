...
public class PAtyLectureGoPaying extends BaseActivity implements
        View.OnClickListener, DoubleBtnFragmentDialog.OnDilaogClickListener, AdapterView.OnItemClickListener {
    ...
    private int balanceNumber;//余额钱数
    ...
    public static final String SERVICETYPEID = "mServiceType";//医生集团服务类型
    public static final String COURSE_ID = "course_id";//医生集团服务类型
    public static final String COURSE_PRICE = "course_price";//医生集团服务类型
...
//        api.handleIntent(getIntent(), this);
        ...
        if (LoginBusiness.getInstance().getLoginEntity() != null) {
            String phoneNum = LoginBusiness.getInstance().getLoginEntity().getPoneNumber();
            String setPsw = LoginBusiness.getInstance().getLoginEntity().getIsSetPwd();
...
//        api.handleIntent(intent, this);
    ...
    private void initPayData() {
        ...
        ApiService.OKHttpQueryYellowBoy(map, new ApiCallbackWrapper<String>(this) {
            @Override
    ...
    private void initDate1() {
        ApiService.OKHttpACCOUNTBALANCE(DoctorHelper.getId(), new ObjectHttpResponseHandler(this) {
            @Override
            ...
            public void onSuccess(Object response) {
                ...
                try {
                    if (response != null) {
                        ...
                        balanceNumber = Integer.parseInt(object.optString("balance"));
                        number.setText("当前余额" + balanceNumber + "元");
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
...
//			send_code = getIntent().getStringExtra("send_code");
        ...
        valuePairs.add(new BasicNameValuePair("CUSTOMERID", LoginBusiness.getInstance().getLoginEntity().getId()));
        valuePairs.add(new BasicNameValuePair("service_item_id", serviceTypeId));
        ApiService.doGetConsultationCouponsCount(valuePairs, new ApiCallback<String>() {
            @Override
            public void onError(Request request, Exception e) {
...
//                } else {
//                    ToastUtil.showShort(PAtyLectureGoPaying.this, bb.message);
//                    couponLayout.setVisibility(View.GONE);
                }
            }
        }, this);
...
//                else
            ...
            case 1://钱包
                ...
                Double b = Double.parseDouble(couponPrice);
                if (a < b) {
...
//            valuePairs.add(new BasicNameValuePair("mServiceType", mServiceType));
//        }
        valuePairs.add(new BasicNameValuePair("op", "buyCourse"));
        valuePairs.add(new BasicNameValuePair("type", "1"));
        valuePairs.add(new BasicNameValuePair("customer_id", DoctorHelper.getId()));
        valuePairs.add(new BasicNameValuePair("course_id", course_id));
        valuePairs.add(new BasicNameValuePair("course_price", course_price));

        ApiService.doGetLectureBuyStudioServlet(valuePairs, new MyApiCallback<String>(this) {
            @Override
            public void onError(Request request, Exception e) {
            ...
            public void onResponse(String response) {
                try {
                    ...
                    isPaying = false;
                    if ("1".equals(obj.optString("code"))) {//成功
                        String source = obj.getJSONObject("result").optString("source");
                        String sign = obj.getJSONObject("result").optString("sign");
                        payZFB(source, sign);
                    } else {
                 ...
                 * 注意这里
                 */
//                ToastUtil.showShort(PAtyLectureGoPaying.this, payBean.message);
...
//				} else {
//					isPaying = false;
//					ResponseBean baseBean = JSONObject.parseObject(response, ResponseBean.class);
...
//				}
    ...
    public void onClickFreePay(View v) {
        ...
        ApiService.doGetConsultationBuyStudioServlet(valuePairs, new ApiCallback<String>() {
            @Override
            public void onError(Request request, Exception e) {
...
//                    return;
     ...
     *{
     "code": "1",
     "message": "银联支付tocken生成成功 ",
     "result": {
     "message": "银联支付tocken生成成功 ",
     "tn": "885676421259487909307",
     "code": 1,
     "pay_id": "1170927291155032"
     }
     }
     * @param v
     */
    public void onClickUnionpay(View v) {
        ...
        ApiService.doGetLectureBuyStudioServlet(valuePairs, new ApiCallback<String>() {
            @Override
            public void onError(Request request, Exception e) {
...
//                        pay_id = baseLove.pay_id;
                        ...
                        // 0 - 启动银联正式环境
                        // 1 - 连接银联测试环境
                        int ret = UPPayAssistEx.startPay(PAtyLectureGoPaying.this, null, null,obj.getJSONObject("result").optString("tn"), mMode);
//                        int ret = UPPayAssistEx.startPay(PAtyLectureGoPaying.this, null, null, baseLove.tn, mMode);
                    ...
                    } else if ("4".equals(obj.optString("code"))) {//0元免费

//                        if ("888".equals(baseLove.role_id)) {
//                        ToastUtil.showShort(PAtyLectureGoPaying.this, baseLove.message);
//                        Intent intent = new Intent(PAtyLectureGoPaying.this, PAytSuccess.class);
...
//                        }
//                        else {
//                            ToastUtil.showShort(PAtyLectureGoPaying.this, obj.optString("message"));
//                        }
...
//            DoubleBtnFragmentDialog.showDoubleBtnPay(PAtyLectureGoPaying.this, "输入支付密码", new DoubleBtnFragmentDialog.OnFirstClickListener() {
//                @Override
...
//                break;
            case R.id.rl_discount_coupon:   //优惠券
                Intent intent = new Intent(PAtyLectureGoPaying.this, PConsultCouponActivity.class);
...
//        final String payInfo = "app_id=2017022405848913&method=alipay.trade.app.pay&format=JSON&charset=UTF-8&sign_type=RSA&timestamp=2017-03-02 13:04:51&version=1.0&notify_url=http://notify_url&biz_content=%7B%22subject%22%3A%22order%22%2C%22out_trade_no%22%3A%22123123123131%22%2C%22total_amount%22%3A%22100%22%2C%22product_code%22%3A%22QUICK_MSECURITY_PAY%22%2C%7D&sign=n4mZyQlQf+aK0GysIUXHMS+tNxaEtikdaKCBVo30HxFz5PhmKsJbr8FjR2T4/F1NZkzLEQwAJOTeJvmaDDiEO0WDL2AVxBbhallrPSQMDnpLuwNZD6dvbvzMjiLsc/HQAeHHnPWQHb2d/GKXTuyb0xvhYAJQK/iHoiCH24pfjVc=";
        ...
        Runnable payRunnable = new Runnable() {
            ...
            public void run() {
                // 构造PayTask 对象
                PayTask alipay = new PayTask(PAtyLectureGoPaying.this);
                // 调用支付接口，获取支付结果
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        Toast.makeText(PAtyLectureGoPaying.this, "支付成功", Toast.LENGTH_SHORT).show();
//                        Intent intent = new Intent(PAtyLectureGoPaying.this, PAytSuccess.class);
//                        if (!HStringUtil.isEmpty(service_type)) {
//                            intent.putExtra(PAytSuccess.ORDER_TYPE, service_type);
//                        }
...
//                        startActivity(intent);
                        ...
                        if (TextUtils.equals(resultStatus, "8000")) {
                            Toast.makeText(PAtyLectureGoPaying.this, "支付结果确认中", Toast.LENGTH_SHORT).show();

                        } else {
                            // 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
                            Toast.makeText(PAtyLectureGoPaying.this, "支付失败", Toast.LENGTH_SHORT).show();
                        }
                    }
                    break;
                }
...
//        Toast.makeText(PAtyLectureGoPaying.this, "获取订单中...", Toast.LENGTH_SHORT).show();
//        try {
...
//                    Toast.makeText(PAtyLectureGoPaying.this, "正常调起支付", Toast.LENGTH_SHORT).show();
//                    // 在支付之前，如果应用没有注册到微信，应该先调用IWXMsg.registerApp将应用注册到微信
//                    api.sendReq(req);
//                } else {
//                    Toast.makeText(PAtyLectureGoPaying.this, "支付失败,请稍后重试", Toast.LENGTH_SHORT).show();
//                }
//            } else {
//                Toast.makeText(PAtyLectureGoPaying.this, "服务器请求错误", Toast.LENGTH_SHORT).show();
//            }
//        } catch (Exception e) {
//            Toast.makeText(PAtyLectureGoPaying.this, "异常:" + e.getMessage(), Toast.LENGTH_SHORT).show();
//        }
        ...
        ApiService.doGetLectureBuyStudioServlet(valuePairs, new ApiCallback<String>() {
            ...
            public void onResponse(String response) {
                try {
                    org.json.JSONObject obj = new org.json.JSONObject(response);
//                    ResponseBean baseBean = JSONObject.parseObject(response, ResponseBean.class);
//                    ResponseBean.Love baseLove = baseBean.love;
                    isPaying = false;
                    if ("1".equals(obj.optString("code"))) {//成功
...
//                        PaySuccessInfoEntity.getInstance().setPay_id(baseLove.pay_id);
                        sendWXPay(obj.getJSONObject("result"));
//                        sendWXPay(baseLove);
                    } else {
                        ToastUtil.showShort(obj.optString("message"));
                    }
    ...
    private void sendWXPay(org.json.JSONObject jsonObject) {
//        try {
//            org.json.JSONObject json=jsonObject.getJSONObject("repmap");
//            PayReq req = new PayReq();
//            req.appId = json.getString("appid");
//            req.partnerId = json.getString("partnerid");
//            req.prepayId = json.getString("prepayid");
...
//        }
    ...
    public void onClickWalletPay(String password) {
        ...
        if (!HStringUtil.isEmptyAndZero(site_id)) {
            valuePairs.add(new BasicNameValuePair("site_id", site_id));
            valuePairs.add(new BasicNameValuePair("mServiceType", service_type_id));
        }
        valuePairs.add(new BasicNameValuePair("op", "buyDoctorService"));
        valuePairs.add(new BasicNameValuePair("doctor_id", doctor_id));
        valuePairs.add(new BasicNameValuePair("service_item_id", serviceTypeId));
        ...
        ApiService.doGetConsultationBuyStudioServlet(valuePairs, new ApiCallbackWrapper<org.json.JSONObject>(this) {
            @Override
            public void onError(Request request, Exception e) {
