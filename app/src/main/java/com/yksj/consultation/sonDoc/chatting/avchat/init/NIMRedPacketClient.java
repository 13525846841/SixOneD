package com.yksj.consultation.sonDoc.chatting.avchat.init;
...
//        JrmfClient.isDebug(false);
    ...
    public static void startWalletActivity(Activity activity) {
        if (checkValid()) {
//            JrmfWalletClient.intentWallet(activity, DemoCache.getAccount(), getThirdToken(), selfInfo.getAccounts(), selfInfo.getAvatar());
        }
    }
...
//            JrmfRpClient.sendGroupEnvelopeForResult(activity, targetAccount, selfInfo.getAccount(), thirdToken, count, selfInfo.getAccounts(), selfInfo.getAvatar(), requestCode);
        } else { // 单聊红包
//            JrmfRpClient.sendSingleEnvelopeForResult(activity, targetAccount, selfInfo.getAccount(), thirdToken, selfInfo.getAccounts(), selfInfo.getAvatar(), requestCode);
        }
...
//            JrmfRpClient.openGroupRp(activity, selfInfo.getAccount(), getThirdToken(), selfInfo.getAccounts(), selfInfo.getAvatar(), briberyId, callBack);
//        } else if (sessionTypeEnum == SessionTypeEnum.P2P) {
//            JrmfRpClient.openSingleRp(activity, selfInfo.getAccount(), getThirdToken(), selfInfo.getAccounts(), selfInfo.getAvatar(), briberyId, callBack);
//        }
    }
...
//        if (checkValid()) {
//            JrmfRpClient.openRpDetail(activity, selfInfo.getAccount(), getThirdToken(), packetId, selfInfo.getAccounts(), selfInfo.getAvatar());
//        }
    }
...
//        if (initialize && selfInfo != null) {
//            JrmfRpClient.updateUserInfo(selfInfo.getAccount(), getThirdToken(), selfInfo.getAccounts(), selfInfo.getAvatar(), new OkHttpModelCallBack<BaseModel>() {
//
//                @Override
