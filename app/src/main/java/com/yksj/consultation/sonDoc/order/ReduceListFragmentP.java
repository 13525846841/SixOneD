...
public class ReduceListFragmentP extends BaseFragment implements PullToRefreshBase.OnRefreshListener2<ListView> {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.reduce_group_fragment_layout, null);
        initView(view);
        return view;
    }
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        groupPersons = getArguments().getString("persons");
    ...
    private void initView(View view) {
        ...
        mAdapter = new ReduceListPAdapter(mActivity, TYPE);
        mListView.setAdapter(mAdapter);
        // mRefreshableView.setOnLastItemVisibleListener(this);
//        getMyFriends(searchContent);
        getData(groupPersons);
    }

    private void getData(String groupPersons) {
        try {
            JSONArray array = new JSONArray(groupPersons);
            ...
            if (count > 0) {
                for (int i = 0; i < count; i++) {
                    ...
                    if (HStringUtil.isEmpty(id) || !id.equals(obj.optString("CUSTOMER_ID"))) {
