 ...
 *
 */
public class SalonSelectPaymentOptionActivity extends BaseActivity implements OnClickSureBtnListener, OnClickListener{
...
//			send_code = getIntent().getStringExtra("send_code");
	...
	protected void onStart() {
		ApiService.doHttpGetQianBao(SmartFoxClient.getLoginUserId(), new AyncHander("WalletBalance"));
		super.onStart();
	}
	...
	public void onClickAlipay(View v){
		if (getIntent().hasExtra("payId") && !getIntent().getExtras().getString("payId").equals("")) {
			...
			ApiService.doHttpBuyTicket(mGroupInfoEntity.getId(),SmartFoxClient.getLoginUserId(),type,"10",new AyncHander("Alipay"));
		}
	}
...
//		ApiService.doHttpGetUnionPay(url, new AyncHander("Unionpay"));
		ApiService.doHttpUnionBuyTicket(mGroupInfoEntity.getId(),SmartFoxClient.getLoginUserId(),type,"20",new AyncHander("Unionpay"));
	}
...
//						{"payId":"130916005122496","ticPrice":1,"code":"10001"}
						if (response.containsKey("payId")) {
							...
							startActivity(intent);
								...
								intent1.putExtra(Constant.Chat.KEY_PARAME, mGroupInfoEntity);
...
//					{"PAY_ID":"1140227007141885","tn":"201402271420460080192"}
	...
	public void onClickSureHander(String pwd) {
		if (TextUtils.isEmpty(pwd)) {
			...
			ApiService.doHttpWalletBuyTicket(MD5Utils.getMD5(pwd),mGroupInfoEntity.getId(),SmartFoxClient.getLoginUserId(),type,"30",new AyncHander("Wallet"));
		}
