...
import com.yksj.consultation.sonDoc.dossier.AtyAddKey;
 ...
 *  医生端会诊病历模板
 */
public class DConsultRecordDocFragment extends BaseFragment implements
		OnClickListener{
	...
	private void initData() {
		ApiService.doHttpConsultionCaseTemplate("4", null, consultionId, new AsyncHttpResponseHandler(getActivity()) {
	...
	private void submitTemplate() {
		...
		ApiService.doHttpConsultionCaseTemplate("5", itemId, consultionId, new AsyncHttpResponseHandler(getActivity()) {
...
//			break;
		case R.id.create_case_keywords_add://编辑关键词

			Intent keyintent =new Intent(getActivity(),AtyAddKey.class);
			keyintent.putExtra("result1",systemTags);
			startActivityForResult(keyintent, TASK_KEY_WORDS_DATA);
			break;
...
//					});
		...
		case R.id.dynamic_case_template_complate://用户输入完成,遍历获取数据,开始提交
			...
			if(verifyData()){//验证完成,提交
				...
				ApiService.doHttpPostConsultionCaseTemplate(params,
...
//			getActivity().overridePendingTransition(R.anim.activity_translate_in, R.anim.activity_translate_out);
	...
	private void onHelpAdd() {
		ApiService.doHttpConsultionCaseTemplate("7", null, consultionId, new AsyncHttpResponseHandler(getActivity()){
			@Override
			public void onSuccess(String content) {
				try {
					JSONObject object=new JSONObject(content);
	...
	private void onPostExpert() {
		...
		ApiService.doHttpSubmitCaseTemplate(params, new AsyncHttpResponseHandler() {
			@Override
			public void onSuccess(String content) {
				try {
					JSONObject object = new JSONObject(content);
	...
	private void onShareCase(String str) {
		...
		params.put("CUSTID", SmartFoxClient.getLoginUserId());
		ApiService.doHttpConsultionCaseTemplateShare(params, new AsyncHttpResponseHandler(){
			@Override
			public void onSuccess(String content) {
				try {
					JSONObject object=new JSONObject(content);
