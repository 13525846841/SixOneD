...
//							context.startActivity(intent);
							break;
//						case -1://话题
//							intent=new Intent(context,FriendCreateTopicListUi.class);
//							intent.putExtra("doctorInfoEntity", mInfoEntity);
//							context.startActivity(intent);
//							break;
						case 2://预约咨询
	...
	public void goBuyService(int serviceId,CustomerInfoEntity mCustomerInfoEntity){
		if (mCustomerInfoEntity != null) {
			if (SmartFoxClient.getLoginUserId().equals(mCustomerInfoEntity.getId())) {
				...
				ApiService.doHttpEngageTheDialogue(SmartFoxClient.getLoginUserId(),mCustomerInfoEntity.getId(),String.valueOf(serviceId) , new AsyncHander(serviceId));
			}
		}
	}
