...
import com.yksj.consultation.sonDoc.chatting.avchat.activity.AVChatExitCode;
import com.yksj.consultation.sonDoc.chatting.avchat.cache.DemoCache;
import com.yksj.consultation.sonDoc.chatting.avchat.constant.CallStateEnum;
import com.yksj.consultation.sonDoc.chatting.avchat.dialog.DialogMaker;
import com.yksj.consultation.sonDoc.chatting.avchat.permission.annotation.BaseMPermission;
import com.yksj.consultation.sonDoc.chatting.avchat.widgets.MultiSelectDialog;
...
public class AVChatUI implements AVChatUIListener {
    ...
    public AVChatUI(Context context, View root,String order_id, AVChatListener listener) {
        this.context = context;
        this.activity = (AVChatActivity) context;
        this.root = root;
...
//        notifyOption.forceKeepCalling = false;
    ...
    private void rejectInComingCall() {
        ...
        AVChatManager.getInstance().hangUp2(avChatData.getChatId(), new AVChatCallback<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                LogUtils.d(TAG, "reject onSuccess-");
            }

            @Override
            public void onFailed(int code) {
                LogUtils.d(TAG, "reject onFailed->" + code);
            }

            @Override
            public void onException(Throwable exception) {
                LogUtils.d(TAG, "reject onException");
            }
        });
    ...
    private void rejectAudioToVideo() {
        AVChatManager.getInstance().sendControlCommand(avChatData.getChatId(), AVChatControlCommand.SWITCH_AUDIO_TO_VIDEO_REJECT, new AVChatCallback<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                LogUtils.i(TAG, "rejectAudioToVideo success");
            ...
            @Override
            public void onFailed(int code) {
                LogUtils.i(TAG, "rejectAudioToVideo onFailed");

            }

            @Override
            public void onException(Throwable exception) {
                LogUtils.i(TAG, "rejectAudioToVideo onException");
            }
        });
    }
    ...
    private void receiveInComingCall() {
        ...
        AVChatManager.getInstance().accept2(avChatData.getChatId(), new AVChatCallback<Void>() {
            ...
            public void onFailed(int code) {
                ...
                LogUtils.e(TAG, "accept onFailed->" + code);
                handleAcceptFailed();
            }

            @Override
            public void onException(Throwable exception) {
                LogUtils.d(TAG, "accept exception->" + exception);
                handleAcceptFailed();
            }
        });
        AVChatSoundPlayer.instance().stop();
    ...
    private void handleAcceptFailed() {
        if (LoginBusiness.getInstance().getLoginEntity() != null && !HStringUtil.isEmpty(LoginBusiness.getInstance().getLoginEntity().getAvAcount())) {
            LoginBusiness.getInstance().getLoginEntity().setAvAcount("");
            SharePreHelper.updateAvChateState(false);
        }
        if (destroyRTC) {
            return;
        }
    ...
    public void videoSwitchAudio() {
        ...
        AVChatManager.getInstance().sendControlCommand(avChatData.getChatId(), AVChatControlCommand.SWITCH_VIDEO_TO_AUDIO, new AVChatCallback<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                LogUtils.d(TAG, "videoSwitchAudio onSuccess");
                //直接切换
                AVChatManager.getInstance().disableVideo();
            ...
            @Override
            public void onFailed(int code) {
                LogUtils.d(TAG, "videoSwitchAudio onFailed");
            }

            @Override
            public void onException(Throwable exception) {
                LogUtils.d(TAG, "videoSwitchAudio onException");
            }
        });
    }
    ...
    public void audioSwitchVideo() {
        ...
        AVChatManager.getInstance().sendControlCommand(avChatData.getChatId(), AVChatControlCommand.SWITCH_AUDIO_TO_VIDEO, new AVChatCallback<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                LogUtils.d(TAG, "requestSwitchToVideo onSuccess");
            ...
            @Override
            public void onFailed(int code) {
                LogUtils.d(TAG, "requestSwitchToVideo onFailed" + code);
            }

            @Override
            public void onException(Throwable exception) {
                LogUtils.d(TAG, "requestSwitchToVideo onException" + exception);
            }
        });
    }
    ...
    private void receiveAudioToVideo() {
        AVChatManager.getInstance().sendControlCommand(avChatData.getChatId(), AVChatControlCommand.SWITCH_AUDIO_TO_VIDEO_AGREE, new AVChatCallback<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                LogUtils.d(TAG, "receiveAudioToVideo onSuccess");
                // 切换
                AVChatManager.getInstance().enableVideo();
            ...
            @Override
            public void onFailed(int code) {
                LogUtils.d(TAG, "receiveAudioToVideo onFailed");
            }

            @Override
            public void onException(Throwable exception) {
                LogUtils.d(TAG, "receiveAudioToVideo onException");
            }
        });
