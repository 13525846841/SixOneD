...
public class ConnectPrintInterceptor implements Interceptor {

    private final boolean isDebug;
    private final Builder builder;

    private ConnectPrintInterceptor(Builder builder) {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (isNotFileRequest(subtype)) {
            ...
            final String url = Printer.getUrlForRequest(response.request());
    ...
    private boolean isNotFileRequest(final String subtype) {
        return !TextUtils.isEmpty(subtype)
                && (subtype.contains("json")
                || subtype.contains("xml")
                || subtype.contains("plain")
                || subtype.contains("html")
                || subtype.contains("x-www-form"));
    }

    private static Runnable createPrintJsonRequestRunnable(final ConnectPrintInterceptor.Builder builder, final Request request) {
        return new Runnable() {
            @Override
            public void run() {
                Printer.printJsonRequest(builder, request);
    ...
    private static Runnable createFileRequestRunnable(final ConnectPrintInterceptor.Builder builder, final Request request) {
        return new Runnable() {
            @Override
            public void run() {
                Printer.printFileRequest(builder, request);
    ...
    private static Runnable createPrintJsonResponseRunnable(final ConnectPrintInterceptor.Builder builder, final long chainMs, final boolean isSuccessful,
    ...
    private static Runnable createFileResponseRunnable(final ConnectPrintInterceptor.Builder builder, final long chainMs, final boolean isSuccessful,
    ...
    public static class Builder {
        ...
        public ConnectPrintInterceptor build() {
            return new ConnectPrintInterceptor(this);
        }
    }
