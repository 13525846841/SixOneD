package com.library.base.widget.richEditor;
...
public class RichTextEditor extends LinearLayout {
    private static final int EDIT_PADDING = 20; // edittext常规padding是10dp
    ...
    public RichTextEditor(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        EditText firstEdit = createEditText("请输入课件内容", dip2px(EDIT_FIRST_PADDING_TOP));
    ...
    private void onBackspacePress(EditText editTxt) {
        ...
        if (startSelection == 0) {
            ...
            // 则返回的是null
            if (null != preView) {
                if (preView instanceof ConstraintLayout) {
                    ...
                    removeView(editTxt);
                    setLayoutTransition(mTransitioner); // 恢复transition动画
                    // 文本合并
    ...
    private EditText createEditText(String hint, int paddingTop) {
        EditText editText = (EditText) inflater.inflate(R.layout.edit_text, this, false);
    ...
    private ConstraintLayout createImageLayout() {
        ConstraintLayout layout = (ConstraintLayout) inflater.inflate(R.layout.edit_imageview, this, false);
    ...
    private void insertImage(Bitmap bitmap, String imagePath) {
        ...
        if (lastEditStr.length() == 0 || editStr1.length() == 0) {
            ...
            if (getChildCount() - 1 == lastEditIndex
                    || editStr2.length() > 0) {
    ...
    private void addEditTextAtIndex(final int index, String editStr) {
        ...
        setLayoutTransition(mTransitioner); // remove之后恢复transition动画
    }
    ...
    private void addImageViewAtIndex(final int index, Bitmap bmp, String imagePath) {
        final ConstraintLayout imageLayout = createImageLayout();
...
//        RelativeLayout.LayoutParams lp = new RelativeLayout.LayoutParams(LayoutParams.WRAP_CONTENT, imageHeight);
